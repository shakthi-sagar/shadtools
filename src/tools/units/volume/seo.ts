...
const UNIT_META: UnitMeta[] = [
  { id: 'liter', singular: 'Liter', plural: 'Liters', slug: 'liter', symbol: 'L' },
  { id: 'milliliter', singular: 'Milliliter', plural: 'Milliliters', slug: 'milliliter', symbol: 'mL' },
  { id: 'gallon', singular: 'Gallon', plural: 'Gallons', slug: 'gallon', symbol: 'gal' },
  { id: 'quart', singular: 'Quart', plural: 'Quarts', slug: 'quart', symbol: 'qt' },
  { id: 'pint', singular: 'Pint', plural: 'Pints', slug: 'pint', symbol: 'pt' },
  { id: 'cup', singular: 'Cup', plural: 'Cups', slug: 'cup', symbol: 'cup' },
  { id: 'fluid-ounce', singular: 'Fluid Ounce', plural: 'Fluid Ounces', slug: 'fluid-ounce', symbol: 'fl oz' },
  { id: 'cubic-meter', singular: 'Cubic Meter', plural: 'Cubic Meters', slug: 'cubic-meter', symbol: 'm³' },
];

function getMeta(id: string): UnitMeta {
  const m = UNIT_META.find((u) => u.id === id);
  if (!m) throw new Error(`Unknown volume unit: ${id}`);
  return m;
}
...
const CONVERSION_PAIRS = [
  ...
  { from: 'gallon', to: 'quart' },
  { from: 'quart', to: 'gallon' },
  { from: 'fluid-ounce', to: 'milliliter' },
...
function buildSlug(v: VolumeVariant): string {
...
function parseSlug(slug: string): VolumeVariant | null {
...
export const volumeSeoProvider: SeoPageProvider<VolumeVariant> = {
  getStaticPages(): VolumeVariant[] {
    const pages: VolumeVariant[] = [];
  ...
  parseVariant(slug: string): VolumeVariant | null {
    return parseSlug(slug);
  },

  getSlug(data: VolumeVariant): string {
    return buildSlug(data);
  },

  isIndexable(data: VolumeVariant): boolean {
  ...
  getMetadata(data: VolumeVariant) {
    const f = getMeta(data.fromId);
    const t = getMeta(data.toId);
    if (data.type === 'pair' || data.value === undefined) {
      return {
        title: `Convert ${f.plural} to ${t.plural} (${f.symbol} to ${t.symbol}) – Volume Converter`,
        description: `Convert ${f.plural.toLowerCase()} to ${t.plural.toLowerCase()} instantly. Free online volume calculator with formula & conversion tables.`,
        h1: `${f.singular} to ${t.singular} Converter`,
      };
    }

    const val = data.value;
    const res = convertVolume(val, data.fromId, data.toId);
    ...
    return {
      title: `${fromFormatted} ${fromLabel} to ${toLabel} – Volume Conversion`,
  ...
  compute(data: VolumeVariant) {
    const val = data.value ?? 1;
    const res = convertVolume(val, data.fromId, data.toId);
    const factor = convertVolume(1, data.fromId, data.toId);
    const f = getMeta(data.fromId);
    const t = getMeta(data.toId);
    const fromLabel = val === 1 ? f.singular : f.plural;
    const toLabel = res === 1 ? t.singular : t.plural;
  ...
  getBreadcrumbParent(data: VolumeVariant): BreadcrumbParent | null {
  ...
  getSections(data: VolumeVariant): SeoSection[] {
    const f = getMeta(data.fromId);
    const t = getMeta(data.toId);
    const val = data.value ?? 1;
    ...
    sections.push({
      ...
      cards: [
        ...
        { label: 'Calculation', value: `${val} × ${factorFormatted} = ${formatNumber(convertVolume(val, data.fromId, data.toId))} ${t.symbol}` },
      ],
    });
    ...
    if (isPairPage) {
      ...
      const rows = sampleValues.map((v) => ({
        from: `${v} ${f.symbol}`,
        to: `${formatNumber(convertVolume(v, data.fromId, data.toId))} ${t.symbol}`,
  ...
  getNearbyVariants(data: VolumeVariant): VolumeVariant[] {
    const val = data.value ?? 1;
